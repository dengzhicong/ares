...
/**
 * 1:定时检测队列 2:队列中时间到的action从新加入执行队列
 * 
 */
public class DelayCheckThread extends Thread {
		...
		while (isRunning) {
			try {
				...
				if (interval > FRAME_PER_SECOND) {
					LogUtil.warn("DelayCheckThread is spent too much time: {} ms, execute = {}", interval, execute);
				}
				if (balance > 0) {
				...
				LogUtil.error("DelayCheckThread error. ", e);
			}
		}
	}
	...
	private int execActors() {
		...
		for (DelayActor delayActor : execQueue) {
			try {
				...
				if (!delayActor.canExec(begin)) {
					addActor(delayActor);
				} else {
					if (!delayActor.getActorQueue().isStop()) {
						delayActor.getActorQueue().enqueue(delayActor);
					}
				}
				executeCount++;
				long end = System.currentTimeMillis();
				if (end - begin > FRAME_PER_SECOND) {
					LogUtil.warn(delayActor.toString() + " spent too much time. time :" + (end - begin));
				}
			} catch (Exception e) {
				LogUtil.error("执行action异常" + delayActor.toString(), e);
			}
		}
		return executeCount;
	}
	...
	private void poll() throws InterruptedException {
		synchronized (lock) {
			if (queue.size() == 0) {
				/**
				 * 千万注意： 当在对象上调用wait()方法时，执行该代码的线程立即放弃它在对象上的锁。然而调用notify()时，
				 * 并不意味着这时线程会放弃其锁。如果线程荣然在完成同步代码，则线程在移出之前不会放弃锁。 因此，只要调用notify()并不意味着这时该锁变得可用。
				 */
				lock.wait();
			} else {
