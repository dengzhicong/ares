			...
			for (int i = 0; i < asyncHandleDataMap.size(); i++) {
				...
				for (int j = 0; j < asyncHandleData.cycleArray.size(); j++) {
					...
					try {
						...
						LogUtil.error("异步线程异常ICycle cycle:" + changeCycle.getClass().getName(), e);
					}
				}
				// 处理本轮加入轮训队列
				for (int j = 0; j < addCycleArray.size(); j++) {
					...
					try {
						...
						LogUtil.error("异步线程异常ICycle onAdd:" + changeCycle.getClass().getName(), e);
					}
				}
				// 处理本轮移除轮训队列
				for (int j = 0; j < removeCycleArray.size(); j++) {
					...
					try {
						...
						LogUtil.error("异步线程异常ICycle onRemove:" + changeCycle.getClass().getName(), e);
					}
				}
				// 处理本轮消息包
