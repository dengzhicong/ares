...
public class FileUtil {
	 ...
	 * @param condition      条件
	 */
	public static void getFiles(String file, List<File> sourceFileList, final String endName,
	 ...
	 * @param condition      条件
	 */
	public static void getFiles(File sourceFile, List<File> sourceFileList, final String endName,
			Predicate<String> condition) {
	...
	/**
	 * 找到指定的目录（前提，目录中不能有重复的目录名）
	 *
	 * @param sourceFile 文件或者目录
	 * @param name       目录名字
	 * @return 返回找到的目录
	 */
	public static File getFileDirectory(File sourceFile, final String name) {
		if (sourceFile.exists()) {
			...
			for (File childFile : childrenFiles) {
				...
				File fileDirectory = getFileDirectory(childFile, name);
	...
	public static String readTxtFile(String filePath) {
		try {
			...
			if (file.isFile() && file.exists()) { // 判断文件是否存在
				...
				LogUtil.warn("文件" + file + "配置有误,找不到指定的文件");
			}
		} catch (Exception e) {
			LogUtil.error("读取文件内容出错", e);
		}
		return null;
	}
	...
	public static boolean deleteFile(String fileName) {
		...
		if (file.exists() && file.isFile()) {
			if (file.delete()) {
				...
				LogUtil.info("删除文件" + fileName + "失败！");
				return false;
			}
		} else {
			LogUtil.info("删除文件失败：" + fileName + "不存在！");
			return false;
		}
	}
